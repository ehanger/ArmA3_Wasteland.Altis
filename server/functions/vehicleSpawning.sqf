...
_totalRadius = 0;
{ _totalRadius = _totalRadius + (_x select 1) } forEach (call tier1ItemLocationList);
_carPerMeters = (["A3W_vehicleQuantity", 200] call getPublicVar) / _totalRadius;
_specialLocationThreads = [];
A3W_vehicleSpawning_specialVehCount = 0;
_startTime = diag_tickTime;

{
	_specialLocationThreads pushBack ([_x, _carPerMeters, _createRandomVehicle] spawn
	{
		_town = _this select 0;
		_carPerMeters = _this select 1;
		_createRandomVehicle = _this select 2;
		...
		{
			_lpos = _pos vectorAdd ([[_maxrad, 0, 0], _langle] call BIS_fnc_rotateVector2D);
			[_lpos, _minrad, _maxrad, A3W_vehicleSpawning_specialVehCount, _specialSpawnedVehicleTypes] call _createRandomVehicle;
			//_minrad = _minrad + 15;
			//_maxrad = _maxrad + 15;
			_langle = _langle + _angleIncr;
			A3W_vehicleSpawning_specialVehCount = A3W_vehicleSpawning_specialVehCount + 1;
			_lcounter = _lcounter + 1;
		};
		//diag_log format["WASTELAND DEBUG - spawned %1 Vehicles in: %2",_lcounter,_townname];
	});
} forEach (call tier1ItemLocationList);

{
	//diag_log format ["Waiting town vehicle spawner #%1", _forEachIndex + 1];
	waitUntil {scriptDone _x};
} forEach _specialLocationThreads;

diag_log format["WASTELAND - Vehicle spawning completed - %1 special vehicles spawned in %2s", A3W_vehicleSpawning_specialVehCount, diag_tickTime - _startTime];
